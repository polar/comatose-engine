...
access a field of a variable by one of two ways.

  Comatose::Variable.define("hey") do
    { "hello" => "Hello World!", "good_bye" => "Goodbye World!" }
  end

And you may access these elements in Liquid/Textile as:
...
+Comatose.register_variable+. You may also refer to these elements in Liquid/Textile the say way as above.

You cannot hand your methods any arguments from Textile as if it were a parameterized function.
In the Liquid implementation, access is always by [] or "fetch" with an integer, "size", "first", and "last".
You can get somewhat creative by overriding [] and "has_key?", but I'll leave that as a hint.
...
* children
This field returns a collection that may be iterated through using a {% for .... %} construct.
