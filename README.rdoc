...
The body of a page is processed by Liquid http\://liquidmarkup.org and RedCloth's Textile http\://redcloth.org/textile.
...
you may access these elements in Liquid as:
...
In this case, you must register an instance of the class under a name that you want to use in your Liquid using
...
In the Liquid implementation security is taken into account so users that may edit pages don't go off running
evals. Access into Liquid variables is restricted to use of [ ], +fetch+ with an integer,
or using +size+, +first+, and +last+ on collections. You may get around this restriction somewhat creatively by
returning another Comatose::Variable by overriding [ ] and +has_key?+, but I'll leave that to the experimenter and
a warning to be careful out there!
...
[+link+] This field returns the url path of a Rails "RESTful" link to the page including the mount point, such as +/home/3422+.
...
That project took a half ton truck, 100 cups of coffee, an angry dog, and two solid weeks of late nights
...
out. I also ran into some bizzare problem with Paperclip in it because of the
use of "responds_to?" and some objects that were getting recycled by a "smart" ruby interpreter.
Anyway, it was way too complicated. Navigating the code was absolutely horrendous. And
...
RubyMine. However, even with great tools, like RubyMine, that doesn't give you license to write crappy code.
...
to show people you're "smart", or probably just the stupid fact that you can do it?
...
repeat yourself"? Really? WTF? So, we should now all write small functions,
...
system almost impossible without a lot of time invested (wasted, IMHO). Especially, if someone else has
to deal with it, whether its a public library or your handing the project off to somebody else at work.
After the debacle with CMS, I've decided to return to Comatose and something
simple. However, even it was too complicated.

So, with the advent of Rails mountable Engines, I've completely stripped Comatose down to its bare essentials.
I've eliminated all the configuration options: authorization models, page caching, and other bells and
...
So, if you want to use this stuff my version, you should be a good programmer, and you should know how to
modify/extend things. It is my hope so that they will work the way you want it to, without
having some obscure configuration option, or covertly modified class crawling up
...
refer to the last engine mounted. Even so, getting the mount point of an engine during a request isn't
yet standard, and getting the functional tests to believe it isn't as sane as I would like it to be.
This situation is as of Rails 3.2.8.

Anyway, be warned, I make no claims for the default app or wish to maintain it to a deployable status.
Use it at your peril. It serves merely as an example.

=== Contributing

If you want to improve upon this project, you can join the obvious Github party, i.e. fork it, and
send pull requests. I would really like to open this up to other ORMs, like MongoMapper, which should be
my next endeavor. However, let me give you this warning:
...
uses "responds_to?", it will be denied. Stick to Obejct Oriented Programming for goodness sake! If you need some
relection and "meta" programming, instead of
"responds_to?" use "is_a?" and a Type name (class name) that guarrantees the method exits.
At least at that point, you really
know what you are dealing with and how calling it may affect the system.

And, last, but not least, use of "method_missing" anywhere will get you duct taped
to a rocket and fired into the virtual abyss to go meet the Flying Spaghetti Monster.

And while your at it. Keep your comments up to date, in proper English, with correct spelling and
